class ProductModal {
    constructor({id, name, description, price, category}) {
        ...
        `<div class="z-10 bg-gray-600/50 h-screen w-screen fixed flex flex-col justify-center items-center">
            <div class="w-fit p-4 bg-slate-900 rounded-md border-2 border-gray-500">
                ...
                <p class="text-white text-lg my-2">Prezzo: {price}€</p>
                <p class="text-white text-lg my-2">Categoria: {category}</p>
                <div class="flex justify-center">
                    <button productid={productId} class="productInfo bg-gray-600 p-2 rounded-lg text-white font-medium align-middle"><a href="https://discord.gg/3jhpqBgB6d">Acquista</a></button>
                </div>
            </div>
         </div>`;
        this.id = id;
    ...
    render() {
        return this.template.replace("{category}", this.category.toUpperCase()).replace("{name}", this.name).replace("{description}", this.description || "Nessuna descrizione presente").replace("{price}", this.price);
    }
}