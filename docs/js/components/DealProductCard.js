class DealProductCard {
    constructor({id, name, description, price, discountPrice, category}) {
        this.template =
        `<div class="product flex flex-col p-4 gap-2 justify-between bg-[#913c3c] rounded-md border-2 border-red-600">
            <b>{name}</b>
            ...
            <button productid={productId} class="productInfo bg-red-500 p-2 rounded-lg text-white font-medium">Dettagli</button>
        </div>`;
        this.id = id;
        this.name = name;
